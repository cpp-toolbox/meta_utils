...
MetaParameter::MetaParameter(const std::string &input) {
    ...
    // const int x becomes "const int" and "x"
    ...
    qualifiers = extract_meta_qualifiers(raw_type_str);
}
...
// TODO: order is not maintained, does it matter?
std::vector<MetaQualifier> extract_meta_qualifiers(const std::string &raw_type) {
    std::vector<MetaQualifier> qualifiers;

    // Word-boundary matches so we don't catch substrings
    static const std::regex const_rx(R"(\bconst\b)");
